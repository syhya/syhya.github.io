...
tags: [LLMs, Parallel Training, Memory Optimization, Data Parallelism, Model Parallelism, Pipeline Parallelism, Tensor Parallelism, MoE, ZeRO, LoRA, AI, Deep Learning]
readingTime: 60
toc: true
ShowToc: true
TocOpen: false
...
> **Note**: This article is **currently being updated** and is still a **draft version**. It will be continuously improved and supplemented. Please follow for the latest version.

## Training Challenges of Large Models

* **Explosive Growth in Parameter Scale**
  In pursuit of higher model capacity and performance, the parameter scale of neural networks is constantly expanding: from millions of parameters to billions, hundreds of billions, and even trillions of parameters, large models are emerging endlessly. For example, Llama 3.1 405B has approximately 405 billion parameters, and it is said that GPT-4 may have reached a parameter scale of 1.7 trillion (1.7T). Such a massive parameter scale brings unprecedented computational and memory pressure to model training.

* **Soaring Computational Complexity**
  The exponential growth in the number of parameters directly leads to a huge increase in computational workload. Training a large model once may take weeks or even months. Even with large-scale high-performance GPU clusters, the training time may still be unacceptable. This severely limits the model iteration speed and research efficiency.

* **Increasingly Prominent Memory Bottleneck**
  During the training of large models, in addition to a large number of model parameters, it is also necessary to store intermediate activation values, gradient information, and optimizer states. These will occupy huge GPU memory. Single-card GPUs (such as A100 or H100 with 80GB of memory, or even H200 with 141GB of memory, GB200 with 384GB of memory) are difficult to cope with models of hundreds of billions or even trillions of parameters. "Out of Memory" (OOM) errors are very common in large model training.

* **Communication Overhead Becomes a Bottleneck**
  In a multi-GPU distributed environment, GPUs need to frequently exchange data for synchronization. As the number of GPUs and model size increase, the communication volume rises significantly, which may become the main bottleneck limiting parallel efficiency. For example, to merge gradients, each All-Reduce operation has to transmit massive amounts of data, which consumes a lot of time even in high-bandwidth networks.

* **Training Stability Challenges**
  Ultra-large-scale models are more prone to gradient vanishing or gradient explosion, and the training process is often unstable and more difficult to converge. Although mixed-precision training can accelerate training and save memory to a certain extent, it may also bring new numerical stability issues, requiring more careful optimization and debugging.


## Necessity of Distributed Training

Faced with the above-mentioned training difficulties of large models, distributed training has become an indispensable solution. Its core idea is to decompose the training task and use multiple computing devices (usually GPU clusters) to work together, making full use of parallel computing power and aggregated memory resources to complete the training. Its advantages are mainly reflected in the following aspects:

* **Breaking Through the Computational Power Limit of a Single GPU**
  The computing power of a single GPU is ultimately limited and cannot meet the computing needs of large models with trillions of parameters. Distributed training can evenly distribute the training load to multiple GPUs, significantly shortening the training time. For example, data parallelism distributes samples in a mini-batch to each GPU for parallel computing, and model parallelism distributes different parts of the model to multiple cards, thereby accelerating overall training.

* **Overcoming the Memory Capacity Bottleneck of a Single GPU**
  The number of parameters, intermediate activation values, and optimizer states of large models often exceed the memory limit of a single card. Distributed training can distribute these data and store them in the memory of multiple GPUs, effectively "merging" more available memory, making it possible to train extremely large-scale models. A typical example is ZeRO technology, which can shard parameters, gradients, and optimizer states and store them on different GPUs.

* **Accelerating Model Iteration and Experiment Cycle**
  With the high parallelism of distributed training, training tasks that originally took weeks or even longer to complete have the opportunity to be completed in days or less. Faster model iteration allows researchers to verify new architectures, new hyperparameters, and new training strategies in a shorter time, thereby accelerating innovation and research and development.

* **Supporting Exploration of Larger-Scale Models**
  Distributed training provides the possibility to explore larger-scale and more complex neural network architectures. The trillion-parameter Switch Transformer can be successfully trained and put into application based on mature distributed training technology.

* **Improving the Robustness and Scalability of the Training System**
  Distributed training systems have good fault tolerance: when a GPU node fails, it can switch to other nodes to continue working, reducing the risk of training interruption. At the same time, the cluster size can also be flexibly expanded or reduced according to needs to adapt to different scales of models and training tasks.
...
- **Data Parallelism (DP)**
  The complete model is copied to each GPU, and the dataset is divided into different batches and distributed to each GPU for parallel computing. Finally, the gradients of all GPUs are aggregated during parameter updates.
...
- **Model Parallelism (MP)**
  ...
  - **Pipeline Parallelism (PP)**: The model is split layer-wise (vertically), with different GPUs responsible for different layers. Micro-batches are passed through the pipeline to execute forward and backward computations in parallel.
  - **Tensor Parallelism (TP)**: Large-scale tensor operations (such as large matrix multiplications) within a layer are split horizontally. Each GPU completes part of the computation in parallel and aggregates when necessary.

- **Expert Parallelism (Expert Parallel)**
  Through a gating strategy, each input sample only passes through a subset of experts (sub-networks), thereby distributing the entire model across different GPUs by "expert modules". Commonly used in Mixture-of-Experts (MOE) structures, it can achieve ultra-large parameter scales but only activate a portion of experts during inference/training.


Below, we will elaborate on these various parallel methods.
...
2. **Partition Training Data**: Divide the large-scale dataset into multiple subsets along the sample dimension, and assign different subsets to different GPUs for processing.
3. **Local Forward and Backward Propagation**: Each GPU independently calculates the loss and the corresponding local gradients.
4. **Gradient/Parameter Synchronization**: Aggregate the gradients from each GPU and update the model parameters to ensure that the model replicas on all GPUs remain consistent after each iteration.
...
5. **Local Backward Propagation**
   Each GPU performs backpropagation based on the local loss $L_i$ to calculate the local gradient
...
7. **Parameter Update**
   Each GPU uses the global average gradient $\bar{g}$ to update the local model parameters:
...
In step 6 "Gradient Synchronization" above, how and when to perform "synchronization" is one of the important factors affecting the performance and convergence behavior of data parallelism. It is generally divided into two major categories:
...
3. **Parameter Update**: Each node uses $\bar{g}$ to update the local parameter replica $\theta$.
4. **Wait and Next Iteration**: All nodes complete the above operations before entering the next iteration.

**Asynchronous Parallel (ASP)** aims to get rid of the global synchronization point of BSP and allow each node to independently perform calculations and parameter updates. Its typical implementation is the **asynchronous push-pull** process under the "Parameter Server" (PS) architecture:

1. Each node calculates the gradient $g_i$ locally, and then **push** it to the parameter server;
2. Once the parameter server receives the gradient, it immediately updates the global model parameters;
3. Other nodes will **pull** down the latest parameters when they need them for the next step of calculation.
...
|:-----------------------|:------------------------------------------------------------------------|:----------------------------------------------------------------------------|
| **Parameter Update Timing** | Global synchronization once per mini-batch or after a certain number of iterations | Each node updates parameters independently, without needing to keep pace with others |
| **Convergence Stability** | **High**. Gradients used are always the latest, convergence path is controllable and easy to analyze | **Lower**. Stale gradients exist, convergence rate and stability may be affected |
| **Communication Needs** | Highly dependent on All-Reduce, all nodes need to wait and exchange data during synchronization | Each node asynchronously pushes/pulls to the parameter server, communication is more flexible, but the parameter server may become a bottleneck |
...
| **Implementation Complexity** | Relatively low, mainstream frameworks (PyTorch DDP, Horovod, etc.) have built-in support | Relatively higher, requires components such as parameter servers, and needs to handle more synchronization logic and data consistency |
| **Applicable Scenarios** | Homogeneous hardware, good network bandwidth, pursuit of higher convergence quality | Heterogeneous hardware, unstable or low bandwidth network, need for extremely high throughput and can tolerate certain convergence risks |
...
Let the gradient of the $j$-th mini-batch be $g_j$. Then, in an "accumulation cycle", we get
...
**Distributed Data Parallel (DDP)** is a highly optimized implementation of BSP ideology in PyTorch v1.5 ([Li et al. 2020](https://arxiv.org/pdf/2006.15704)), providing convenience for data parallelism in single-machine multi-GPU and even multi-machine multi-GPU scenarios. Its main optimizations include:

1. **Gradient Bucketing**: Divide model parameters into multiple "buckets"; when backpropagation is performed, once all gradients in a bucket are calculated, an **All-Reduce for that bucket** is immediately initiated, instead of waiting for all gradients to be calculated before synchronizing all at once.
2. **Communication and Computation Overlap**: DDP uses asynchronous communication and non-blocking operations to overlap gradient synchronization (communication) with forward propagation and backpropagation (computation) as much as possible, thereby reducing communication overhead. This overlapping strategy improves overall parallel efficiency.
3. **Gradient Accumulation**: DDP can also be easily combined with **gradient accumulation**. Combined use reduces the synchronization frequency by increasing the gradient update interval for each synchronization. In large-scale distributed training, this helps to further reduce communication overhead and improve training efficiency.
...
In a multi-GPU environment (especially single-machine multi-GPU), if there is high-speed interconnect (such as NVLink, PCIe switch, etc.), **Ring All-Reduce** can be used to significantly reduce communication overhead. Its idea is:

1. Organize $k$ nodes into a ring and divide the gradient vector into $k$ equal parts.
2. In the "summation phase", each node sends a part of its local gradient to the next node and adds it to the received gradient; after several iterations of this process, each node will hold the complete "aggregated" gradient.
3. In the "broadcast phase", the final result is distributed to all nodes along the ring.

Ideally, the communication cost of Ring All-Reduce is approximately independent of the number of nodes (it can be regarded as $\mathcal{O}(1)$), which is very suitable for gradient synchronization in a multi-GPU environment. It is a core communication mode widely used in libraries such as Horovod and NCCL.

### Parameter Server

When the cluster scale expands to multi-machine multi-GPU, if simple single-point aggregation (such as a central server) is used, it is often difficult to support parallel training of massive data. **Parameter Server (PS)** ([Li, et al., 2014](https://www.usenix.org/system/files/conference/osdi14/osdi14-paper-li_mu.pdf)) is a typical architecture designed for scalable distributed training:
...
2. **push-pull Semantics**: After a computing node obtains the gradient locally, it **pushes** it to the corresponding PS. After the PS updates the parameters of the shard, the computing node can **pull** down the latest version when needed for the next step of calculation.
3. **Flexible Fault Tolerance and Expansion**: By adding or removing PS nodes, the capacity can be flexibly expanded in terms of bandwidth or computing needs. Backup and fault tolerance strategies can also be implemented on the PS.

This **PS + Worker** mode can combine data parallelism and model parallelism **simultaneously**, splitting ultra-large models and storing them on multiple PSs, and performing distributed training on ultra-large data. The PS itself can also be split and merged according to the load, forming a more complex hierarchical topology.
...
Naive model parallelism implementation, which simply divides the model layer by layer and executes it sequentially on different GPUs, will encounter a serious "bubble" problem. Due to the dependencies between layers, when one GPU is processing a certain stage of a certain data sample, other GPUs may be idle, waiting for the output of the previous GPU or the input of the next GPU. This GPU idle time is called "bubble", which seriously reduces the efficiency of pipeline parallelism.
...
* **Sequential execution:** Naive model parallelism executes layer by layer in order, which prevents GPUs from working in full parallel.
...
GPipe ([Huang et al. 2018](https://arxiv.org/abs/1811.06965)) is an efficient pipeline parallel training system proposed by Google, designed to solve the bubble problem of naive pipeline parallelism. The core idea of GPipe is to divide a **mini-batch** into multiple **micro-batches** and use **synchronous gradient aggregation** to alleviate the bubble problem and improve pipeline efficiency.
...
2. **Pipeline Stage Partitioning:** Divide the model layer by layer into $d$ stages, and each stage is assigned to a GPU.
...
5. **Parameter Update:** Each GPU uses the global average gradient to update the local model parameters.
...
When the number of micro-batches $m$ is much larger than the pipeline depth $d$ ($m \gg d$), the bubble ratio approaches 0, and the pipeline efficiency approaches linear acceleration. The GPipe paper points out that when $m > 4d$, the bubble overhead is almost negligible (in the case of activation recomputation). Therefore, there are the following benefits:
...
2. **Pipeline Stage Partitioning:** Divide the model layer by layer into $d$ stages, and each stage is assigned to a GPU.
3. **1F1B Scheduling Execution:** Each GPU takes turns performing forward propagation and backward propagation.

### Weight Stashing

Since forward propagation and backward propagation in 1F1B scheduling may use different versions of model weights, it will cause weight version inconsistency problems, affecting the correctness and convergence of training. PipeDream introduces Weight Stashing technology to solve this problem. The core idea of weight stashing is that each GPU maintains multiple versions of model weights and ensures that forward propagation and backward propagation use the same version of weights.

**Weight Stashing Implementation:**

* **Version Management:** Each GPU maintains a weight version queue, storing multiple versions of model weights.
...
* **Version Update:** After completing the backward propagation of all micro-batches in a mini-batch, update the model weights and generate a new weight version.
...
PipeDream-flush periodically performs global synchronous pipeline flushing on the basis of PipeDream, similar to GPipe's synchronous gradient aggregation. Through periodic flushing, PipeDream-flush can greatly reduce the memory space required for weight stashing, only needing to maintain a single version of model weights, but it will sacrifice a small amount of throughput.


### PipeDream-2BW

PipeDream-2BW (Double-Buffered Weights) maintains two versions of model weights, namely "double-buffered weights". It updates the model version every $k$ micro-batches, where $k$ is greater than the pipeline depth $d$ ($k > d$). The newly updated model version will not immediately completely replace the old version, because there may still be some remaining backward propagation operations that still depend on the old version. Through double-buffered weights, PipeDream-2BW can reduce the memory overhead of weight stashing to only maintaining two versions of model weights, significantly reducing memory usage.
...
**Tensor Parallelism (TP)** is a parallel method that splits tensors (usually weight matrices) in the model along specific dimensions and distributes the split shards to different GPUs for computation. Tensor parallelism has the following advantages:

* **Breaking Through Single GPU Memory Limits:** Tensor parallelism can distribute model parameters and store them on multiple GPUs, breaking through the memory capacity limit of a single GPU and supporting the training of larger-scale models.
...
* **Combining with Data Parallelism and Pipeline Parallelism:** Tensor parallelism can be used in combination with other parallel technologies such as data parallelism and pipeline parallelism to form multi-dimensional parallel strategies, further improving training efficiency and scalability.
...
The MLP layer of Transformer usually contains two linear layers. The computation of the first linear layer can be expressed as $Y = \text{GeLU}(XA)$, where $X$ is the input matrix, $A$ is the weight matrix, and GeLU is the activation function. Megatron-LM splits the weight matrix $A$ along the column dimension into $P$ shards $[A_1, A_2, ..., A_P]$, where $P$ is the number of GPUs. Each GPU $i$ is responsible for storing and computing the weight shard $A_i$.

**MLP Layer Tensor Parallel Computation Process:**
...
**Self-Attention Layer Tensor Parallelism**

Megatron-LM also performs tensor parallel splitting on the Query ($Q$), Key ($K$), Value ($V$) weight matrices in the Transformer's self-attention layer, and performs corresponding local matrix multiplication and global concatenation operations to achieve tensor parallelism of the self-attention layer. The calculation formula of the self-attention layer is:
...
* **Multi-dimensional Parallel Combination:** PTD-P uses pipeline parallelism, tensor parallelism, and data parallelism at the same time, which can parallelize the training process from multiple dimensions.
...
**Mixture-of-Experts (MoE)** ([Shazeer et al., 2017](https://arxiv.org/abs/1701.06538)) is a sparsely activated model that significantly increases the number of model parameters and performance without significantly increasing the computational cost by combining multiple independent "expert" networks and a gating network. The core idea of MoE is **Sparse Activation**, that is, for each input sample, only a part of the expert network is activated, rather than the entire model. This method not only improves computational efficiency but also enhances the expressive ability of the model, making it perform well in LLMs.

MoE's design inspiration comes from [Ensemble learning](https://en.wikipedia.org/wiki/Ensemble_learning), a technology that decomposes complex tasks into multiple subtasks and completes them by different models in collaboration. In MoE, these "subtasks" are processed by multiple independent expert networks, and the gating network is responsible for dynamically selecting the most suitable experts based on the characteristics of the input samples. This division of labor and cooperation mechanism is similar to an expert team in human society: experts in different fields provide professional opinions for specific problems, and finally, a comprehensive result is obtained.
...
* **Gating Network:** A trainable neural network $G$ used to learn a probability distribution based on the input sample $x$ to decide which experts to activate. The input of the gating network is the input sample $x$, and the output is an $n$-dimensional probability vector $p = G(x) = [p_1, p_2, ..., p_n]$, where $p_i$ represents the probability of activating expert $E_i$.
...
In order to achieve sparse activation and ensure balanced expert usage, MoE usually adopts **Noisy Top-k Gating** as the gating mechanism. This method ensures computational efficiency and avoids the problem of uneven expert load by introducing noise and Top-k selection. The following is its detailed workflow:


1. **Gating Score Calculation:**

For an input sample \(x\), the gating network first calculates the gating score \(H^{(i)}(x)\) for each expert. This score consists of two parts: a linear transformation and a noise term, as shown in the formula:
$$
...
- **Parameter Description**:
  ...
  - \(\text{softplus}(x) = \log(1 + e^x)\): Smooth activation function to ensure non-negative noise.
...
Setting the scores of non-Top-k experts to \(-\infty\) ensures that the probabilities of these experts are 0 in the subsequent softmax operation, achieving sparsity.
...
$$
Only the probabilities of the Top-k experts are non-zero, and the rest are 0. For example, if \(n=100, k=2\), the probabilities of 98 experts are 0.
...
Switch Transformer ([Fedus et al. 2021](https://arxiv.org/pdf/2101.03961)) is a MoE model proposed by Google with a parameter scale reaching **trillions**. Its core innovation is to replace the dense feed-forward network (FFN) layer in the Transformer model with a sparse Switch FFN layer. Different from GShard's Top-2 Gating, Switch Transformer only routes each input token to one expert network, which has higher sparsity and further reduces the computational cost, making it possible to train trillion-parameter models. Encourage token routing to be more balanced among \(N\) experts. The auxiliary loss of Switch Transformer is based on the accumulation of the product of the actual routing ratio and the predicted routing probability. The specific formula is as follows:
...
- **Parameter Description**:
  ...
  - \(f_i\): Proportion of tokens routed to the \(i\)-th expert, defined as:
    $$
    f_i = \frac{1}{T} \sum_{x \in B} 1\{\text{argmax } p(x) = i\}
    $$
  - \(P_i\): Routing probability of the \(i\)-th expert predicted by the gating network, defined as:
    $$
...
1. **Routing Prediction:** For an input token $x$, Switch Router predicts the routing probability $p_i = G^{(i)}(x)$ of each expert network, where $i = 1, 2, ..., n$, and n is the number of expert networks.
2. **Expert Selection:** Select the expert network with the highest routing probability as the best expert network. Switch Transformer adopts the Top-1 routing strategy, that is, each token is only routed to the expert network with the highest routing probability.
3. **Token Routing:** Route the input token $x$ to the selected best expert network for processing.
...
- **Selective Precision**
  Using FP32 precision inside the routing function can improve training stability and avoid the additional overhead caused by FP32 tensor communication. Specifically, the calculation process of Switch Router uses FP32 throughout, and the final result is converted to FP16 to balance efficiency and precision.
...
- **Higher Expert Dropout**
  Using a higher dropout rate (e.g., 0.4) in the expert FFN layer and a lower dropout rate (e.g., 0.1) in non-expert layers can effectively prevent overfitting and enhance the generalization ability of the model. The experimental results in the figure below show that the model performs best when the dropout rate of the expert layer is set to 0.4 on tasks such as GLUE, CNNDM, SQuAD, and SuperGLUE.
...
The Switch Transformers paper uses the following figure to intuitively show how different parallel technologies divide model weights and data:
...
Expert Choice (EC) ([Zhou et al. 2022](https://arxiv.org/abs/2202.09368)) is a routing strategy opposite to token selection routing (such as GShard's top-2 or Switch Transformer's top-1). In token selection routing, each token selects top-$k$ experts from all experts for routing; while in expert choice routing, each expert selects top-$k$ tokens from all tokens for processing. This method aims to solve the problems of load imbalance and token waste in token selection routing, and significantly improve training efficiency. The following is the specific calculation process:

1. **Calculate token-to-expert affinity scores**
...
2. **Experts choose tokens**

   Each expert selects top-$k$ tokens from all tokens for processing. By performing top-$k$ selection on $S^T$:
...
EC controls the sparsity of the model by regularization to limit the number of experts to which each token is routed. A common regularization target is as follows:
...
The optimization problem considered defines a matrix \(A\), whose element in the \(i\)-th row and \(j\)-th column indicates whether the \(i\)-th expert has selected the \(j\)-th token (value 0 or 1). Since this optimization problem is complex to solve, the paper uses the [Dykstra algorithm](https://en.wikipedia.org/wiki/Dijkstra%27s_algorithm) (to obtain an approximate solution through multiple iterations) to solve it.

The parameter \(b\) is usually determined by the total number of tokens \(n\) in the batch and the capacity factor, where the capacity factor represents the average number of experts used by each token. Most experiments use a higher capacity factor. The experimental results show that even when the capacity is reduced, EC (Expert Choice) still performs better than the traditional top-1 token selection routing, although capped expert choice slightly reduces the fine-tuning performance.

The advantages of EC are mainly reflected in the following two aspects:
- **Perfect load balancing:** Each expert processes a fixed number of \(k\) tokens, thus avoiding the problem of some experts being overloaded while others are idle, and achieving ideal load balancing.
- **Higher training efficiency:** Experiments show that EC can improve the training convergence speed by about 2 times, which is more efficient than traditional token selection routing.
...
1. **Sequence Chunking**
   Divide the input sequence into several chunks, and each chunk is saved and computed by different GPUs; therefore, each card only needs to store the activation of its corresponding sequence chunk, avoiding single-card memory explosion.
2. **Ring Communication + Self-Attention**
   Propose the Ring Self-Attention (RSA) mechanism: each GPU first locally calculates local attention, and then sequentially transmits (ring structure) Key/Value chunks to adjacent GPUs. After multiple iterations, it is guaranteed that each GPU can obtain global sequence information.
...
1. **Sequence Dimension Splitting**
   For activations such as LayerNorm and Dropout that are difficult to split in the tensor dimension, divide them along the sequence dimension, so that each GPU only processes a part of the sequence's non-linear operations.
...
DeepSpeed-Ulysses ([Jacobs et al. 2023](https://arxiv.org/abs/2309.14509)) proposes an efficient sequence parallelism scheme for ultra-long sequence training. By partitioning the input in the sequence dimension and combining it with two-stage all-to-all communication, it effectively reduces communication volume and activation memory, thereby supporting the training of million-token-level long sequence Transformer models.
...
2. **Two-Stage Communication Optimization**
   - **First All-to-All:** Perform all-to-all exchange on Q/K/V before attention calculation to distribute activation calculation and reduce memory pressure per card;
   - **Second All-to-All:** After attention calculation, collect output context and remap it to local sequence partitions, which not only restores the original sequence structure but also significantly reduces the amount of communication data.

3. **Efficient Communication and Generality**
   Using all-to-all communication reduces the communication volume to \(O(N/P)\), which saves nearly \(P\) times the bandwidth compared to the traditional All-Gather method (communication volume \(O(N)\)); at the same time, this scheme is suitable for dense and sparse attention and can be seamlessly integrated with ZeRO-3 memory optimization, thereby supporting efficient training of larger models and longer sequences.
...
- **Model States:** Including **optimizer states** (such as Adam's momentum and second-order moments), **gradients**, and **model parameters**. Mixed-precision training not only requires storing FP16 data but also needs to retain FP32 versions of parameters and states, resulting in higher memory usage.
...
To solve the problem of memory redundancy, ZeRO adopts two major strategies:

1. **ZeRO-DP (Data Parallelism):**
   For model states, eliminate redundancy by sharding and distributing optimizer states, gradients, and parameters to multiple data parallel processes, and use dynamic communication scheduling to reduce communication volume.

2. **ZeRO-R (Residuals Optimization):**
   For activations and temporary buffers, optimize memory usage by using sharded activation recomputation, fixed buffer size, and real-time memory fragmentation management.

### ZeRO Sharding Strategy

ZeRO is divided into three stages. Each stage further reduces memory redundancy on the basis of the previous stage, thereby making it possible to train ultra-large models:

#### ZeRO-1 (Optimizer State Sharding)
   - **Principle:**
     - Divide optimizer states (such as Adam's momentum and second-order moments) into \(P\) shards along the parameter dimension (\(P\) is the number of GPUs), and each GPU only stores the states corresponding to the model parameters it is responsible for.
     - Local update: Each GPU only updates its locally stored state and parameter shards during the parameter update stage, without additional cross-GPU communication.
...
#### ZeRO-2 (Gradient Sharding)
   - **Principle:**
     ...
     - Each GPU calculates local gradients, uses efficient Reduce-Scatter operation to aggregate gradients, and then updates local parameter shards.
...
### DeepSpeed ZeRO Sharding and Offload Strategy Comparison
...
| **ZeRO-1**     | Only optimizer states are sharded, gradients and parameters are still replicated. | Higher | Slightly slower than ZeRO-0 |
...
|----------------------------------|----------|--------------|--------------|
| **ZeRO-1 + CPU Offload**         | On the basis of ZeRO-1, offload optimizer states to CPU memory, reducing GPU memory footprint, but depends on PCIe bandwidth and occupies CPU memory. | Medium-Low | Slower than ZeRO-1 |
| **ZeRO-2 + CPU Offload**         | On the basis of ZeRO-2, offload optimizer states to CPU memory, further reducing GPU memory for large models, but increasing CPU-GPU data transfer. | Low | Slower than ZeRO-2 |
| **ZeRO-3 + CPU Offload**         | On the basis of ZeRO-3, offload optimizer states and model parameters to CPU, GPU memory footprint is the lowest, but CPU-GPU communication overhead is extremely large. | **Extremely Low** | **Very Slow** |
| **ZeRO-Infinity (NVMe Offload)** | Based on ZeRO-3, offload states to NVMe devices, breaking through CPU memory limits, suitable for ultra-large models; performance highly depends on NVMe parallel read and write speed. | **Extremely Low**<br/>NVMe support required | Slower than ZeRO-3, but usually better than CPU Offload scheme |
...
- **ZeRO-3:**
  Need to perform All-Gather/All-Reduce operations on model parameters, the communication volume increases significantly, and network bandwidth becomes a key bottleneck.
...
- **Computational Efficiency:**
  To make the number of computing workers exceed the limitations of simple model and pipeline parallelism, and to ensure computational efficiency, DeepSpeed expands with ZeRO-DP (data parallelism based on optimizer state sharding). ZeRO-DP not only further optimizes memory usage but also greatly reduces communication overhead by topologically aware mapping of data parallel groups to devices with local high-bandwidth communication.
...
- **Optimize Intra-node Communication:** Since model parallelism has the largest communication overhead, model parallel groups are preferentially arranged within the same node to utilize higher intra-node bandwidth (for example, using NVIDIA Megatron-LM's tensor splitting method);
...
To further expand the model scale, Llama3 ([Grattafiori et al., 2024](https://arxiv.org/abs/2407.21783)) adopted a 4D parallel strategy during training. It combines four parallel methods to shard the model more finely, so that the model parameters, optimizer states, gradients, and activations on each GPU can adapt to the capacity limits of high-bandwidth memory (HBM). These four parallel methods are:
...
- **Data Parallelism (DP), usually using Fully Sharded Data Parallelism (FSDP):** Shard the model, optimizer states, and gradients, and synchronize after each training step.
...
Through the 4D parallel strategy, Llama3 can fully utilize the computing resources of multiple GPUs during training, while effectively reducing memory usage and supporting the training of ultra-large-scale models.



## Memory Optimization Techniques

In addition to parallel training techniques, there are many memory optimization techniques designed to help train LLMs. These designs mainly start from reducing the memory usage of each stage in the training process.

### CPU Offloading

CPU Offloading ([Rhu et al. 2016](https://arxiv.org/abs/1602.08124)) refers to a common and intuitive practice of unloading data or tensors that are temporarily not needed to the CPU when GPU memory is insufficient and loading them back to the GPU when needed. Its main purpose is to use the larger capacity of CPU memory to expand the available space, so that larger-scale models can be trained even in a memory-constrained environment. However, this method will bring additional data transfer overhead and usually reduce the training speed, so its application has relatively decreased in recent years.

1. **Identify Offloadable Tensors:** Identify tensors that are temporarily not used during training, such as model parameters, optimizer states, intermediate activation values, etc. The basis for judging whether a tensor can be offloaded can be the usage frequency, life cycle, etc. of the tensor.
2. **Tensor Offloading:** Move offloadable tensors from GPU memory to CPU memory. Data transfer is usually performed through the PCIe bus.
...
Activation Recomputation/Gradient Checkpointing ([Chen et al. 2016](https://arxiv.org/abs/1604.06174)) is a **technology that trades computation for memory**. During training, only some activation values are saved (for example, the input activation values of each Transformer layer). During backpropagation, the unsaved activation values are recalculated. Activation recomputation can significantly reduce the memory footprint of activation values during training, especially when training deep neural networks.

1. **Select Checkpoints:** Select some layers in the model as checkpoints. Usually, key layers in the model are selected, such as the input layer of the Transformer layer.
2. **Forward Pass:** During the forward pass, only the activation values of checkpoint layers are saved. For non-checkpoint layers, the activation values are immediately released after calculation and not saved.
3. **Backward Pass:** During the backward pass, when it is necessary to calculate the gradient of a non-checkpoint layer, first perform a forward pass again to calculate the activation value of that layer (recomputation), and then perform backward propagation to calculate the gradient. For checkpoint layers, since the activation values of checkpoint layers have been saved, the saved activation values can be directly used for backpropagation without recomputation.
...
Next, consider how to choose the optimal $k$ to minimize given $n$:
...
Therefore, the overall peak memory requirement of this approach can be reduced to the order of $O(\sqrt{n})$ (compared to the $O(n)$ memory of generally directly saving all activations). This is why activation recomputation technology can bring "sublinear" memory usage. The following figure intuitively shows the effect of this trick.

{{< figure
    src="activation_recomputation.png"
    caption="Fig. 27. The memory cost of different memory saving algorithms. Sharing: Memory used by intermediate results is recycled when no longer needed. Inplace: Save the output directly into memory of an input value. (Image source: [Chen et al. 2016](https://arxiv.org/abs/1604.06174))"
    align="center"
    width="100%"
>}}


It should be noted that activation recomputation requires additional forward recomputation in the **backward propagation** stage. Each segment needs to do a forward calculation of $n/k$ layers. If the network is divided into $k$ segments, the total recomputation during backpropagation is approximately $k \times \bigl(n/k\bigr) = n$ layers of forward operations, which is equivalent to doing approximately one more "forward propagation" in each training iteration. This is usually acceptable in LLM training because:

- Compared to quickly exhausting GPU memory and making it impossible to train large-scale models, this additional computational cost is usually more bearable.
...
1. **Full-Precision Master Copy of Weights**
   To prevent gradients from being truncated to zero due to being too small in magnitude under FP16, a full-precision FP32 master copy of weights is maintained during training. The specific process is:
...
2. **Loss Scaling**
   ...
   - Multiply the loss by the scaling factor \(S\) to get \(L' = L \times S\) and then perform backpropagation to calculate FP16 gradients;
   ...
   As shown in the figure below, amplifying the loss makes the gradient distribution more concentrated in the higher numerical value part, thereby retaining the subtle information that may be truncated under low-precision representation.

{{< figure
    src="mixed_precision_fp16.png"
    caption="Fig. 29. The histogram of gradients in full precision. The left part up to \(2^{-24}\) will be zero-ed off once the model switches to FP16. (Image source: [Micikevicius al. 2017](https://arxiv.org/abs/1710.03740))"
    align="center"
    width="100%"
>}}
...
In the deep learning training process, although intermediate results (such as activation values and gradient information) are only used once in a forward propagation and once in a backward propagation, they often occupy a lot of memory. Considering that there is a significant time interval between two uses, the data can be **compressed** after the first use, and then decompressed when needed later, thereby effectively reducing memory usage.

Compression technology is mainly applied in two scenarios:

- **Activation Value Compression:** Compress activation values after forward propagation and decompress them before backpropagation. This is especially important for deep neural networks because activation values usually occupy a lot of memory.
...
2. **Lossy Compression:**
   Algorithms such as JPEG or MPEG are used to obtain higher compression rates on the premise of allowing certain data loss. This method can significantly reduce memory usage, but may have a certain impact on model accuracy and convergence.

Gist ([Jain et al. 2018](https://www.microsoft.com/en-us/research/uploads/prod/2018/04/fiddle-gist-isca18.pdf)) is a memory optimization technology for activation value compression. Its core lies in using data encoding strategies to compress intermediate results. It mainly includes two encoding schemes:

- **Layer-Specific Lossless Encoding:**
  Design specialized lossless encoding schemes for specific layer structures (such as ReLU-Pool and ReLU-Conv):
...
- **Aggressive Lossy Encoding:**
  Adopt Delayed Precision Reduction (DPR) technology. The core idea of DPR is: activation values need to maintain high precision during forward propagation, while lower precision can be tolerated during backpropagation. Therefore, activation values are compressed to lower precision after forward propagation, and then decompressed to high precision before backpropagation.
...
Adafactor ([Shazeer et al. 2018](https://arxiv.org/abs/1804.04235)) is a memory-efficient adaptive learning rate optimizer. Unlike Adam, Adafactor does not store the complete second-order moment estimation matrix, but only stores two vectors (row and column statistics) to replace the complete second-order moment matrix, which significantly reduces memory usage, especially suitable for scenarios where the parameter matrix has a low-rank structure.

#### SM3

SM3 (Sparse Momentum for Massive Models) ([Anil et al. 2019](https://arxiv.org/abs/1905.11286)) provides a similarly memory-efficient adaptive optimization scheme through sparse updates and state sharing.
...
LoRA (Low-Rank Adaptation) ([Hu et al., 2021](https://arxiv.org/abs/2106.09685)) proposes to introduce **low-rank adapters** next to pre-trained weights, and achieve efficient fine-tuning by adding a small number of parameters without interfering with the original inference ability of the pre-trained model.
...
{{< figure
    src="lora.png"
    caption="Fig. 30. An illustration of LoRA (Image source: [Hu et al., 2021](https://arxiv.org/abs/2106.09685))"
    align="center"
    width="70%"
>}}

Assume that the pre-trained weight matrix is \(\mathbf{W} \in \mathbb{R}^{d \times k}\). LoRA adds a low-rank update term \(\Delta \mathbf{W} = \mathbf{B}\mathbf{A}\) to it, thereby obtaining new weights:
...
During fine-tuning, **the original weight \(\mathbf{W}\) remains frozen**, and only \(\mathbf{A}\) and \(\mathbf{B}\) are updated. Since \(r\) is much smaller than \(d\) or \(k\), the number of parameters that need to be trained is greatly reduced.


To ensure that the introduced \(\Delta \mathbf{W} = \mathbf{B}\mathbf{A}\) in the early stage of fine-tuning has as little impact on the original model as possible, \(\Delta \mathbf{W}\approx 0\) is required. Common practices are as follows:
...
- **No Additional Inference Latency:** After training is completed, the update term \(\Delta \mathbf{W}\) can be merged back into the original weights, which will not increase the computational load during the inference stage.


### QLoRA

QLoRA ([Dettmers et al., 2023](https://arxiv.org/abs/2305.14314)) is a method for efficient fine-tuning of large-scale models based on LoRA combined with quantization ideas. Through the following three key improvements, it greatly reduces memory footprint while keeping model accuracy basically unchanged:

1. **4-bit Normal Float (NF4) Quantization**
   Adopt a block-based quantile quantization strategy to quantize the original model weights to 4 bits, thereby achieving significant storage compression with subtle loss of accuracy.
...
3. **Paged Optimizer**
   When memory usage is too high, automatically transfer part of the optimization process to CPU memory, thereby reducing GPU memory pressure and improving scalability.

Different from traditional LoRA which only reduces the number of parameters to be fine-tuned, QLoRA also **compresses** all weights through 4-bit quantization, thereby maximizing the reduction of memory footprint and data transfer overhead while ensuring near-original accuracy.

{{< figure
    src="qlora.png"
    caption="Fig. 31. Different finetuning methods and their memory requirements. QLoRA improves over LoRA by quantizing the transformer model to 4-bit precision and using paged optimizers to handle memory spikes. (Image source: [Dettmers et al., 2023](https://arxiv.org/abs/2305.14314))"
    align="center"
    width="100%"
>}}

This method can be regarded as a further extension of LoRA: LoRA improves efficiency by reducing the number of weights that need to be fine-tuned, while QLoRA, on this basis, quantizes all weights (including the un-fine-tuned parts) to 4-bit precision, achieving **dual compression of storage and computation** in general, which is suitable for efficient fine-tuning of LLMs in resource-constrained environments.


## Summary

Parallel techniques and memory optimization strategies need to be weighed and selected according to the specific model structure, dataset size, hardware resources, and training goals. In general, it is necessary to combine multiple technologies to effectively train large-scale models and achieve the best performance and efficiency.
