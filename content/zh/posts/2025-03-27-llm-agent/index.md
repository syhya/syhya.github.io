...
- **实时获取信息：** 利用搜索引擎 Google、Bing 或数据库 API，模型可以访问最新信息，确保生成内容的时效性和准确性。
...
**HuggingGPT** ([Shen, et al. 2023](https://arxiv.org/abs/2303.17580))是一种利用 ChatGPT 作为任务规划器的框架，通过读取模型描述从 [Hugging Face](https://huggingface.co/) 筛选可用的模型来完成用户任务，并根据执行结果进行总结。
...
CUA 和之前现有的 WebVoyager 不同之处在于这是一个专门经过强化学习训练的 Agent，而不是直接调用 GPT-4o 搭建的固定流程的 Workflow。尽管 CUA 目前仍处于早期阶段且存在一定局限，但它已在多个基准测试中取得 SOTA 结果，如下图所示。
...
**OpenAI Deep Research**（[OpenAI, 2025](https://openai.com/index/introducing-deep-research/)）是 OpenAI 于 2025 年 2 月正式发布的一款智能 Agent，专为复杂场景设计，能自动搜索、筛选、分析并整合多源信息，最终生成高质量的综合报告。该系统以 [o3](https://openai.com/index/introducing-o3-and-o4-mini/) 为核心基座，并结合强化学习方法，显著提升了多轮迭代搜索和推理过程中的准确性与稳健性。
...
- **上下文窗口限制**：LLM 的上下文窗口限制了信息处理量，影响长期规划和记忆能力，降低任务连贯性。当前研究探索外部记忆机制和上下文压缩技术，以增强长期记忆及复杂信息处理能力。截至本文写作时（2025 年 3 月），OpenAI 最新发布的模型 **GPT-4.5**([OpenAI, 2025](https://openai.com/index/introducing-gpt-4-5/)) 最大上下文窗口为 128k tokens。
