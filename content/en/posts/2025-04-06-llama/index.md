...
*   **LLaMA 3.2 (2024/10):** Introduced **lightweight models** (e.g., 1B, 3B) optimized for edge devices (like phones, watches, smart homes), and released **multimodal vision models** (e.g., Llama-3.2-11B-Vision and Llama-3.2-90B-Vision).
...
2.  **Reward Modeling (RM):** Utilize the collected human preference data triplets (prompt, chosen, rejected) to train one or more reward models. The goal of the reward model is to learn to predict the degree of human preference for model-generated responses, providing a quantitative signal for subsequent optimization. LLaMA 3 trained a single reward model covering different capabilities (including Helpfulness and Safety).
...
| **Tokenizer**         | SentencePiece (32k)  | SentencePiece (32k)  | SentencePiece (32k)  | Based on LLaMA 2/3   | tiktoken (128k)      | tiktoken (~200k)          |
