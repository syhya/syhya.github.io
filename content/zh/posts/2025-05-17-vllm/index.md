...
这种机制显著减少了**并行采样 (Parallel Sampling)** 的内存开销，论文实验显示可节省约 6.1%-9.8%（Alpaca）至 16.2%-30.5%（ShareGPT）的内存。
...
vLLM 通过引用计数和 CoW 机制，高效地管理这种动态共享，避免了传统实现中频繁且昂贵的内存拷贝操作。大部分块可以共享，只有当新生成的 token 落入旧的共享块时才需要 CoW（仅拷贝一个块）。论文实验显示，束搜索场景下可节省高达 37.6%-55.2% 的内存。
...
> Yue Shui. (May 2025). vLLM：高吞吐、内存高效的 LLM 服务引擎.
