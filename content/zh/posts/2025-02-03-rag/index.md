...
categories: ["技术博客"]
description: "系统梳理检索增强生成（RAG）的基础流程、检索技术、范式演进、GraphRAG、Agentic RAG、多模态 RAG、评估方法与应用边界。"
summary: "本文系统梳理 RAG 的基础流程、核心检索技术、范式演进、GraphRAG、Agentic RAG、多模态 RAG、评估方法与应用边界。"
...
3.  **领域知识和长尾知识缺乏**: 对于特定垂直领域或私有知识库，通用 LLMs 可能缺乏足够的专业知识。相关研究指出 **LLMs 在学习长尾知识方面存在困难**([Kandpal et al., 2023](https://arxiv.org/abs/2211.08411))，而 RAG 可以通过外部知识库在一定程度上缓解这类问题，但效果取决于知识覆盖、索引质量和检索召回。
...
为了克服这些局限性，**检索增强生成（Retrieval-Augmented Generation, RAG）** 技术应运而生。[Lewis et al. (2020)](https://arxiv.org/abs/2005.11401) 最早系统提出将参数化记忆（语言模型参数）与非参数化记忆（外部检索语料）结合的 RAG 框架，并区分了 RAG-Sequence 与 RAG-Token 两类生成方式。今天更常见的工程化 RAG 则通常指在推理时将外部检索结果注入上下文，从而提升模型输出的质量、时效性和可信度。
...
{{< figure
    ...
    alt="RAG 问答流程：文档索引、查询检索、上下文组装和答案生成"
    align="center"
    width="90%"
>}}
...
*   **文本嵌入 (Text Embedding)**: 使用预训练的文本嵌入模型（如 OpenAI 的 `text-embedding-3-small` / `text-embedding-3-large`）将每个文本块转换为高维向量。这些向量能够捕捉文本块的语义信息，使得语义相似的文本块在向量空间中距离更近。查询 \( q \) 也会被同一个嵌入模型转换为向量 \( \mathbf{v}_q = E(q) \)，而每个文本块 \( c \) 被转换为 \( \mathbf{v}_c = E(c) \)。
...
{{< figure
    ...
    alt="Flat、LSH、HNSW 和 IVF 等向量索引算法的性能与资源权衡"
    align="center"
    width="100%"
>}}
...
2.  **相似性搜索 (Similarity Search)**: 在向量数据库中，利用索引结构高效地查找与查询向量 \( \mathbf{v}_q \) 最相似的文本块向量 \( \mathbf{v}_c \)（实际使用 ANN 算法避免逐一比较）。最常用的相似度度量是**余弦相似度 (Cosine Similarity)**：
    ...
    *   **欧氏距离 (Euclidean Distance / L2 Distance)**:
        \[
        \begin{aligned}
        L_2(\mathbf{v}_q, \mathbf{v}_c)
        &= \|\mathbf{v}_q - \mathbf{v}_c\| \\
        &= \sqrt{\sum_{i=1}^{n} (v_{q_i} - v_{c_i})^2}
        \end{aligned}
        \]
        距离越小表示越相似。
...
{{< figure
    ...
    alt="RAG 范式从朴素 RAG 到高级 RAG 再到模块化 RAG 的演进"
    align="center"
    width="100%"
>}}
...
*   **后检索策略 (Post-retrieval Strategies)**: 这些策略在检索到初步结果后进行优化。
    ...
    *   **上下文压缩与选择 (Context Compression and Selection)**:
        ...
        *   **信息过滤**: 过滤掉与查询主题无关或质量较低的检索结果。FILCO ([Wang, Z. et al., 2023](https://arxiv.org/abs/2311.08377)) 学习过滤上下文以增强 RAG。
...
*   **交互模式 (Interaction Patterns)**:
    ...
    *   **自适应检索 (Adaptive Retrieval)**:
        *   **Self-RAG** ([Asai et al., 2024](https://arxiv.org/abs/2310.11511)): LLM 在生成过程中动态决定是否需要检索以及检索什么内容。它会生成检索令牌（如 `[Retrieve]`）和批判令牌（如 `ISREL`、`ISSUP`、`ISUSE`）来评估是否需要外部知识、检索段落是否相关、答案是否被证据支持以及输出是否有用。如果判断需要检索，它会生成检索查询，获取信息，然后继续生成。
...
\begin{aligned}
\text{score}(Q, D)
= \sum_{q_i \in Q} \text{IDF}(q_i) \cdot
...
{{< figure
    ...
    alt="GraphRAG 使用 LLM 抽取实体关系、构建图谱、检测社区并生成全局答案"
    align="center"
    width="100%"
>}}
...
{{< figure
    ...
    alt="Agentic RAG 中代理的记忆、规划、工具使用和反思等核心组件"
    align="center"
    width="90%"
>}}
...
*   **Search-R1** ([Jin et al., 2025](https://arxiv.org/abs/2503.09516)): 使用 RL 训练 LLM 在逐步推理过程中自主生成搜索查询并进行实时检索。引入 retrieved token masking 实现稳定的 RL 训练；在论文报告的七个 QA 数据集和相同检索设置下，Qwen2.5-7B 相比若干 RAG baseline 有显著提升。

*   **R1-Searcher** ([Song et al., 2025](https://arxiv.org/abs/2503.05592)): 纯 RL 方法（无需过程奖励或蒸馏），教会 LLM 在推理过程中自主调用搜索。其优势应限定在论文实验的 QA 基准、检索环境和评测指标内，不应理解为通用模型能力排序。
...
{{< figure
    ...
    alt="多模态 RAG 流程：多模态数据处理、跨模态检索、融合与生成"
    align="center"
    width="100%"
>}}
...
5. **多模态生成技术 (Multimodal Generation Techniques)**
   ...
   * **上下文学习 (In-Context Learning, ICL)：** 利用检索到的多模态内容作为少样本示例，无需重新训练即可增强 MLLM (如 GPT-4V ([OpenAI, 2023](https://openai.com/index/gpt-4v-system-card/)), LLaVA ([Liu, H. et al., 2023](https://arxiv.org/abs/2304.08485)), Gemini ([Gemini Team, 2024](https://arxiv.org/abs/2312.11805))) 的推理能力。
...
*   **ViDoRAG** ([Wang, Q. et al., 2025](https://arxiv.org/abs/2502.18017)): 面向视觉文档的多智能体 RAG 系统，使用基于 GMM 的混合检索结合文本和视觉特征。采用迭代式智能体工作流（探索、总结、反思），比现有方法提升超过 10%。
...
*   **CoRAG (Chain-of-Retrieval Augmented Generation)** ([Wang, L. et al., 2025](https://arxiv.org/abs/2501.14342), Microsoft): 训练“o1-like RAG 模型”，通过拒绝采样生成中间检索链，实现逐步检索与推理。在论文报告的多跳问答设置下提升 10+ EM 分数，并展示了 RAG 的测试时计算扩展能力。
...
*   **HippoRAG 2** ([Gutierrez et al., 2025](https://arxiv.org/abs/2502.14802)): 将 RAG 推向类人长期记忆系统，使用增强的 Personalized PageRank 实现更深层的段落整合。在论文报告的关联记忆任务中，比相关 SOTA 嵌入模型提升 7%，同时在事实和语义理解任务上表现优异。
...
*   **ROUGE (Recall-Oriented Understudy for Gisting Evaluation)** ([Lin, 2004](https://aclanthology.org/W04-1013/)): 基于 N-gram 召回率。
    ...
    \text{ROUGE-N}
    = \frac{
    \sum_{S \in \{\text{RefSummaries}\}}
    \sum_{\text{gram}_n \in S} \text{Count}_{\text{match}}(\text{gram}_n)
    }{
    \sum_{S \in \{\text{RefSummaries}\}}
    \sum_{\text{gram}_n \in S} \text{Count}(\text{gram}_n)
    }
    \end{aligned}
    \]
...
1. **检索质量决定上限。** 分块、嵌入、BM25/向量混合检索、RRF 和重排序共同决定了 LLM 能看到什么证据；生成模型再强，也很难稳定弥补低召回、低相关或冲突的上下文。
2. **RAG、长上下文和微调各有边界。** RAG 更适合动态知识、私有知识和需要溯源的场景；长上下文适合少量长文档的整体理解；微调更适合学习格式、风格和任务行为。实际系统常常需要组合使用。
3. **GraphRAG、Agentic RAG 和多模态 RAG 扩展了问题空间。** 图结构有助于全局主题和实体关系推理，Agentic RAG 适合多步骤工具调用，多模态 RAG 则让表格、图片、视频和版面信息进入检索与生成链路。
4. **评估必须覆盖端到端风险。** 除了 Recall@K、NDCG、faithfulness、answer relevance 等离线指标，还需要关注引用错配、上下文冲突、提示注入、语料污染、延迟和在线用户任务成功率。
...
[3] OpenAI. ["GPT-4V(ision) System Card."](https://openai.com/index/gpt-4v-system-card/) OpenAI System Card (2023).
...
[60] Hsiao, Chi-Hsiang, et al. ["MegaRAG: Multimodal knowledge graph-based RAG."](https://arxiv.org/abs/2512.20626) ACL 2026.
...
[84] Wang, Zhiruo, et al. ["Learning to filter context for retrieval-augmented generation."](https://arxiv.org/abs/2311.08377) arXiv preprint (2023).
...
[88] Zhou, Junjie, et al. ["MegaPairs: Massive data synthesis for universal multimodal retrieval."](https://arxiv.org/abs/2412.14475) arXiv preprint (2024).
...
[98] Wu, Junda, et al. ["CoRAL: Collaborative retrieval-augmented large language models for recommendation."](https://arxiv.org/abs/2403.06447) arXiv preprint (2024).
...
[101] Lewis, Patrick, et al. ["Retrieval-Augmented Generation for Knowledge-Intensive NLP Tasks."](https://arxiv.org/abs/2005.11401) NeurIPS 2020.

[102] Gao, Yunfan, et al. ["Retrieval-Augmented Generation for Large Language Models: A Survey."](https://arxiv.org/abs/2312.10997) arXiv preprint (2023).
