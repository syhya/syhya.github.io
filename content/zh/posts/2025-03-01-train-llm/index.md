...
最近大模型的参数数量不断攀升，从最初的数十亿扩展到如今数千亿乃至数万亿级别。大模型虽然带来了前所未有的应用效果，但与此同时，也引发了计算资源、内存管理和训练稳定性等一系列严峻挑战。因此本博客总结了一些常用分布式并行训练和内存管理技术，希望能够帮助大家更好地训练和优化大模型。
...
为了避免门控网络过度偏向少数专家，MoE 引入了**辅助损失(Auxiliary Loss)**([Shazeer et al. 2017](https://arxiv.org/abs/1701.06538))。其中 importance loss 基于各专家门控权重之和的[变异系数(Coefficient of Variation, CV)](https://en.wikipedia.org/wiki/Coefficient_of_variation)平方：
...
- **专家容量(Expert Capacity):**  
  为了避免专家过载，GShard 为每个专家设置可处理 token 数量的上限。每个 token 依次尝试分发给两个候选专家；两个候选专家的容量均耗尽时，该 token 被标记为 "overflowed"，MoE 门控输出退化为零向量，并通过残差连接传递到下一层。
...
- **辅助损失(Auxiliary Loss):**  
  GShard 希望减小各专家路由比例的均方，但实际 token 计数来自不可微的 top-2 操作。因此，实际辅助损失使用“路由到专家的 token 比例”与“该专家的平均门控权重”的乘积作为可微近似。
...
- **选择性精度(Selective Precision)**  
  在路由函数内部采用 FP32 精度既能提高训练稳定性，又能避免因 FP32 张量通信而产生的额外开销。具体来说，Switch Router 的局部计算使用 FP32，生成的 dispatch 和 combine 张量再转换回 BF16，以兼顾效率与精度。
...
- **原理：**  
  ...
  - 局部更新：每个 GPU 在参数更新阶段仅更新其负责的参数分片；更新完成后通过 All-Gather 交换这些分片，使所有 GPU 获得完整且一致的更新后参数。

#### ZeRO-2(梯度分片) 
- **原理：**  
  - 在优化器状态分片的基础上，将梯度沿参数维度同样进行分片，每个 GPU 只存储对应梯度分片。  
  - 每个 GPU 计算局部梯度，通过 Reduce-Scatter 将归约后的梯度分片交给对应 GPU，再更新本地参数分片；随后通过 All-Gather 同步更新后的参数。

#### ZeRO-3(参数分片)
- **原理：**  
  - 在 ZeRO-1 和 ZeRO-2 的基础上，将模型参数(通常为 16 位数据)也进行分片，每个 GPU 只存储与其对应的参数分片。  
  - 按需参数收集：在每层的前向和反向计算前，通过 All-Gather 临时重建该层参数，使用后即可释放；反向传播产生的梯度则通过 Reduce-Scatter 归约并保留对应分片。
...
|----------------|----------|--------------|--------------|
| **ZeRO-0**     | 纯数据并行，不进行任何分片，所有状态在每个 GPU 上完全复制。 | 最高 | 普通 DP 基线 |
| **ZeRO-1**     | 仅分片优化器状态，梯度和参数仍复制。 | 较高 | 通信量与普通 DP 相同，性能取决于通信调度和实现 |
| **ZeRO-2**     | 分片优化器状态和梯度。 | 中等 | 通信量与普通 DP 相同，使用梯度 Reduce-Scatter 和参数 All-Gather |
| **ZeRO-3**     | 分片优化器状态、梯度和模型参数。 | 最低 | 通信量最多约为普通 DP 的 1.5 倍，对网络带宽更敏感 |
...
| **ZeRO-Infinity(NVMe Offload)** | 基于 ZeRO-3，将状态卸载到 NVMe 设备，突破 CPU 内存限制，适合超大模型；性能高度依赖 NVMe 并行读写速度。 | **极低**；需 NVMe 支持 | 在其他条件相近时通常慢于 CPU Offload，但可利用更大的 NVMe 容量 |

### 通信量与性能影响

- **ZeRO-0/1/2：**  
  设模型参数量为 $\Psi$，原论文按每个数据并行进程在单步中发送和接收的数据量计算：普通 DP 的梯度 All-Reduce 通信量为 $2\Psi$，ZeRO-1/2 的总通信量同样为 $2\Psi$。以 ZeRO-2 为例，梯度 Reduce-Scatter 和更新后参数 All-Gather 的通信量各为 $\Psi$。
  
- **ZeRO-3：**  
  前向和反向传播分别按需 All-Gather 参数，并通过 Reduce-Scatter 归约梯度，总通信量为 $3\Psi$，最多约为普通 DP 的 1.5 倍。参数通信采用 All-Gather，梯度通信采用 Reduce-Scatter；实际性能影响取决于模型、通信调度和网络带宽。
...
1. **权重全精度主副本**  
   训练过程中保留一份 FP32 权重主副本。FP32 的尾数精度更高，可以在参数更新时保留加到权重上的小幅变化。具体流程为：
   ...
   这种设计兼顾了低精度前向与反向计算的效率，以及参数更新所需的数值精度。
...
2. **有损压缩(Lossy Compression):**  
   面向训练张量的常见方法包括低比特量化或编码，以及对梯度进行稀疏化(如 top-$k$)。有损梯度压缩还可配合误差反馈(error feedback)，将本轮未传输的残差累积到后续迭代；激活值则可采用下文 Gist 的 DPR 等方法。
