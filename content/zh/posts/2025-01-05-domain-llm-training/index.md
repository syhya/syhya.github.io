...
- **CPU Offload**（ZeRO-1/2/3 + CPU）
  ...
  - 通信量主要来自 **CPU 与 GPU 之间**的数据传输，带宽远低于 GPU-GPU 通信，极易造成性能瓶颈，尤其在 **ZeRO-3** 场景下。
...
| 直接偏好对齐 (DPO)   | 利用偏好数据（正例 chosen vs. 负例 rejected）直接对齐人类偏好 | 偏好数据：[prompt, chosen, rejected]（规模相对较小） | 对齐人类反馈，如回答风格、合规性、安全性等  |
